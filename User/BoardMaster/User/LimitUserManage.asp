...
Function SpecialUserBrowser
	...
	If Num>0 Then
		...
		if FirstID > MinRecordID and FirstID<>0 then
			PageSplictString = PageSplictString & " <a href=LimitUserManage.asp" & EndwriteQueryString & "&Start=" & LngStr(FirstID) & "&UpDownPageFlag=1&SubmitFlag=3829EwoqIaNfoG>上页</a> " & VbCrLf
		else
			'PageSplictString = PageSplictString & " <font color=999999 class=grayfont>上页</font> " & VbCrLf
		end if
	
		if LastID<MaxRecordID and LastID<>0 then
			PageSplictString = PageSplictString & " <a href=LimitUserManage.asp" & EndwriteQueryString & "&Start=" & LngStr(LastID) & "&SubmitFlag=3829EwoqIaNfoG>下页</a> " & VbCrLf
		else
			'PageSplictString = PageSplictString & " <font color=999999 class=grayfont>下页</font> " & VbCrLf
		end if
	
		if LastID < MaxRecordID and LastID<>0 then
			PageSplictString = PageSplictString & " <a href=LimitUserManage.asp" & EndwriteQueryString & "&Start=" & LngStr(MaxRecordID+1) & "&UpDownPageFlag=1&SubmitFlag=3829EwoqIaNfoG>尾页</a> " & VbCrLf
		else
...
Function DisplayNewForbidIP
	...
	' Upstream typo: the guard read "= 10", a value DEF_EnableForbidIP never takes (it is 0 or
	' 1), so with IP blocking switched OFF this page still offered the moderator a full block
	' form — which then wrote a row that inc/Board_Popfun.asp never enforces. The standalone
	' NewForbidIP.asp gets the same test right (= 0).
