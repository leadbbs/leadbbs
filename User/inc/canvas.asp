...
' ***************************************************************************
 ...
 ' README §41: a multi-index `Property Let` does not reliably write the class field under
 ' AxonASP — the assignment is discarded with no error, so EVERY pixel written to this canvas
 ...
 public sub SetPixel(ByVal lX,ByVal lY,ByVal lValue)
  ...
  ' `Int(var)` is written `Int(var + 0)` on purpose — see README §41: AxonASP's Int/Abs/Sqr/
...
public sub Circle(ByVal lX,ByVal lY,ByVal lRadius)
 ...
 private function DrawVectorChar(ByVal lX,ByVal lY,sChar,lSize,bOrientation)
  ...
  ' Bitmap font support
 ' README §42: each glyph is stored as its 31 rows joined with "|", because AxonASP's
 ...
 ' asked about — DrawText walks one character at a time.
...
Public Sub DrawTextWE(ByVal lX,ByVal lY,sText,space)
  ...
  dim sTemp,lOffset,lXa,lYa,lValue,tt
	...
	For iTemp1 = 1 to t2
		...
		' `+ 0` deliberately (README §41): ABS() of a bare variable/array element gets a slot
		' index instead of the value, so this guard never fired and the per-row shear divisor
		' could be 1 or 2 — which smeared every glyph into a diagonal streak.
			...
			If inStr(t7,"1") > 0 Then
				...
				For iTemp3 = 1 to t3
					...
					if bChar <> "0" then
						...
						' AxonASP (README §41). The clipping is SetPixel's job too — note the
...
End Sub
 ...
 ' Clear the image.
 '
 ' README §41. The original line was
 ...
 ' every captcha on the site — no human could register or post:
 '
 ...
 '   2. AxonASP strings are BYTE strings — LenB() equals Len() — so even with an integer
...
' ***************************************************************************
 ...
 private function UncompressedData()
  ...
  UncompressedData = ""
  ' README §41 again: `^` yields a DOUBLE, and AxonASP's MidB() returns "" when its length
  ' argument is one — silently, so the chunk loop emitted nothing but clear codes and the GIF
