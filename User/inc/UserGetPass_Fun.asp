	...
	Public Sub DisplayReset
		...
		If Request.Form("act") = "reset" Then
			If Len(pass1) < DEF_UserShortestPassword or Len(pass1) < DEF_UserShortestPassword or pass1 <> pass2 Then
				Call VierForm("新的密码不能少于" & DEF_UserShortestPassword & "位，并且新密码与重复密码必须相同。")
				Exit Sub
			End If
			...
			set rs = ldexecute(sql,0)
			if rs.eof then
				Call VierForm("此用户名（或邮箱、手机)从未注册过.")
				rs.close
				set rs = nothing
				exit sub
			end if
			...
			if get_sessionid < 1000 then
				Call VierForm("您未获得过重置验证码，或重置验证码已无效，请重新获取.")
				exit sub
			end if
			...
			if errNum > 50 then
				Call VierForm("您的失败次数已超过50次，重置验证码不再有效，需要重新获取才能重置.")
				tmp = markReplace(remark,2,0)
			...
			If get_sessionid <> AttestNumber Then
				...
				Call VierForm("重置验证码错误，您还有" & (51-errNum) & "次尝试机会.")
				Exit Sub
			End If
