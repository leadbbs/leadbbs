...
if GetBinarybit(DEF_Sideparameter,23) = 1 then 'https auto redirect
	if LCase(request.ServerVariables("HTTPS")) = "off" Then
		...
		' README §51: always True on a bare comparison, so this appended a "?" to every URL.
...
-->
	...
	<ul class="list_line">
		...
		<li><a href="#33" onclick="fAddFavorite('<%=DEF_SiteNameString%>','<%=htmlencode(LD_GetUrl(2)) & "?" & htmlencode(Replace(Request.ServerVariables("QUERY_STRING"),"&","&amp;"))%>')">加入收藏</a></li>
	...
	Select Case Ptype
		Case 1:
			...
			elseif id <= 1 then
				...
				' README §46: toNum() hands back a Double, and AxonASP renders that as
