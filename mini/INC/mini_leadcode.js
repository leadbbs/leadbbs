...
var auto_nextplay = 1; //auto to play next radio
...
function lrc_create(obj,tt,auto,width,height)
{
	...
	switch(clas)
	{
		...
		case "swf":
			...
			{
				str='<div id="jplayer">'+_close+'<iframe height=\"498\" width=\"96%\" style=\"max-height:96%;\" src=\"' + ur + '\" frameborder=0 allowfullscreen></iframe>';
			}
			else
			{
...
function lrc_start(obj,tt,auto,w,h){
	...
	$("#jquery_jplayer_1").jPlayer({
		...
		timeupdate: function(event) {
			...
			ended: function() {
			if(auto_nextplay==1)
			if($("#jplayer").prev("div").attr("class").indexOf("lrc_item_audio")!=-1)
			{
				if($("#jplayer").parent().nextAll(".ubb_audio").length>0)
				$("#jplayer").parent().nextAll(".ubb_audio").eq(0).find(".lrc_source").click();
				else
				{
					var cur = $("#jplayer").parents(".announcecontent").nextAll(".announcecontent");
					while($(cur).length>0)
					{
						if($(cur).find(".ubb_audio").length>0)
						{
						$(cur).find(".ubb_audio").eq(0).find(".lrc_source").click();
						break;}
						else
						cur = $(cur).nextAll(".announcecontent");
					}
				}
				}
			},
