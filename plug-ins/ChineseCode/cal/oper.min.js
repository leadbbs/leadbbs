�?function () {
	...
	$("#record-list-con").delegate("p", "click", function (e) {
		...
		stitle = d.data[day][ind].title || "请添加主题，最�?44个汉�?,
		...
		try {
			...
			if (!dl || dl.length == 0) {
				if ($("#recordListNull").length === 0) {
					$("#record-list-con").html('<span id="recordListNull" style="margin: 6px 5px;color: #666;display: block;">今日无记�?/span>');
				}
			} else {
...
function callA() {
	...
	var a = new Date();
	a.setTime(c);
	document.getElementById("result1").innerHTML = a.getFullYear() + "�? " + (1 + a.getMonth()) + "�? " + a.getDate() + "�?   星期" + weekDays(a.getDay());
}
function callB() {
	var b = true;
	...
	document.getElementById("result2").innerHTML = Math.abs(Math.round((c - e) / 86400000)) + " �?;
}
$(function () {
