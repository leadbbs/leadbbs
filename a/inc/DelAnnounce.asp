...
Function CheckDelSure
	...
	If GetBinarybit(GBL_CHK_UserLimit,5) = 1 Then
		Call Processor_ErrMsg("你已经被" & LimitUserStringData(4) & "！" & VbCrLf)
		CheckDelSure = 0
		Exit Function
	End If
	...
	If GetBinarybit(GBL_Board_BoardLimit,5) = 1 Then
		Call Processor_ErrMsg("此版面不允许删除帖子！")
		CheckDelSure = 0
		Exit Function
	End If
...
Sub Process_DelAnnounce(AncID)
	...
	If Rs.Eof Then
		Call Processor_ErrMsg("错误，未选择要删除的帖子！" & VbCrLf)
		Rs.Close
		Set Rs = Nothing
		Exit Sub
	End if
	...
	If GBL_Board_ID <> 444 and DEF_EnableDelAnnounce = 0 and ParentID = 0 Then
		Call Processor_ErrMsg("系统已经禁止直接删除主题帖子，请使用回收站功能。")
		Exit Sub
	End If
	Dim DelPoints
		If ParentID > 0 Then
		...
		Else
			...
			LMT_AncID = 0
			Call Processor_Done("<br>成功删除此主题相关的回复帖子及主题帖子")
		End If
		...
		End If
		CALL LDExeCute("insert into LeadBBS_Log(LogType,LogTime,LogInfo,UserName,IP,BoardID) Values(101," & GetTimeValue(DEF_Now) & ",'" & Left("成功删除版面编号" & LngStr(BoardID) & "，帖子编号" & LngStr(LMT_AncID) & "，作者编号" & UserID & "的帖子．标题内容：" & Replace(AnnounceTitle,"'","''"),255) & "','" & Replace(GBL_CHK_User,"'","''") & "','" & Replace(GBL_IPAddress,"'","''") & "'," & GBL_Board_ID & ")",1)
