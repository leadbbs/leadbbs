...
Function CheckTypeSetSure
	...
	Dim UserID
	If LMT_AncID = 0 Then
		Call Processor_ErrMsg("错误，请提供要自动排版的帖子的ID！" & VbCrLf)
		CheckTypeSetSure = 0
		Exit Function
	End if
	Dim Rs,SQL
	...
	If Rs.Eof Then
		Call Processor_ErrMsg("错误，未选择要处理的帖子！" & VbCrLf)
		Rs.Close
		Set Rs = Nothing
		CheckTypeSetSure = 0
		Exit Function
...
Sub DisplayTypeSetAnnounce
	...
	If Request.Form("SureFlag")="1" Then
		If CheckWriteEventSpace() = 0 Then
			Call Processor_ErrMsg("<font color=red class=redfont>您的操作过频，请稍候刷新再试！</font>")
			Exit Sub
		End If
		Select Case DoingFlag
			Case 1:	If Form_NotReplay = 0 Then
						Form_NotReplay = 1
						Call Processor_Done("成功锁定帖子。")
					Else
						Form_NotReplay = 0
						Call Processor_Done("成功完成帖子解锁。")
					End If
			...
			Case 2:	If Form_TitleStyle >= 60 Then	
					...
					Call Processor_Done("帖子成功通过审核操作。")
				Else
					Form_TitleStyle = Form_TitleStyle + 60
					Form_Title = "新审核帖子..."
					If inStr(application(DEF_MasterCookies & "TopAncList"),"," & LMT_AncID & ",") Then
				...
				If Form_AncUserID > 0 and inStr(LCase(DEF_SupervisorUserName),"," & LCase(Form_AncUserName) & ",") = 0 Then
					If Form_TitleStyle <> 30 Then
						If GetBinarybit(Form_UserLimit,3) = 1 or GetBinarybit(Form_UserLimit,7) = 1 Then
							Call Processor_Done("该用户已被禁言或屏蔽发言，不需要重复操作！")
						Else
							...
							Call Processor_Done("成功禁言用户：" & htmlencode(Form_AncUserName) & "，并标记此帖！")
						End If
					Else
						...
						Call Processor_Done("成功解除禁言用户：" & htmlencode(Form_AncUserName) & "，并解除标记此帖！")
					End If
				Else
					Call Processor_Done("无足够权限，操作已终止！")
				End If
			Case Else:
				GBL_CHK_TempStr = ""
				...
				Else
					Call Processor_Done("成功对帖子完成自动排版。")
				End If
		End Select
	Else
		Processor_Head()
	...
	%>>
			...
			<%Else%>
			<b>确认要自动排版编号为<font color=ff0000 class=redfont><%=LngStr(LMT_AncID)%></font>的帖子内容吗？</b>
			<%End If
