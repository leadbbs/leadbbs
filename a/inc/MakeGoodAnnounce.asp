<%
' README §41: `Abs(x)` reads a storage slot instead of the value when x is a bare variable
' under AxonASP, so every Abs() below is written `Abs(x + 0)` — an expression argument is
' evaluated correctly. It matters here: these are the points deducted from and awarded to a
' user for rating a post, and the range clamps that keep 评帖 inside DEF_BBS_PrizeAnnouncePoints.
...
Function CheckMakeGoodSure
	...
	If GetBinarybit(GBL_CHK_UserLimit,6) = 1 Then
		Call Processor_ErrMsg("错误，权限不足！" & VbCrLf)
		CheckMakeGoodSure = 0
		MakeGood_Level = 0
		Exit Function
	End if
	...
	If Rs.Eof Then
		Call Processor_ErrMsg("错误，要精华的帖子ID不存在！" & VbCrLf)
		Rs.Close
		Set Rs = Nothing
		CheckMakeGoodSure = 0
		MakeGood_Level = 0
	...
	End If
	If isArray(Temp) = False Then
		Call Processor_ErrMsg("错误论坛发生错误，请联系管理员！" & VbCrLf)
		CheckMakeGoodSure = 0
		MakeGood_Level = 0
		Set Rs = Nothing
		Exit Function
	...
	Else
		CheckMakeGoodSure = 0
		MakeGood_Level = 0
		Call Processor_ErrMsg("错误，权限不足(UsrLMT)！")
	End If
...
Sub DisplayMakeGoodAnnounce
	...
	If MakeGood_Level = 1 or (Request.Form("Form_GoodType") = "2" and DEF_MasterNolimit = 0) Then
		If Opinion_CheckAllowOpinion() = 0 Then
			Call Processor_ErrMsg("<span class=redfont>您对此帖评价次数已超出次数限制,或是评价值不足.</span>")
			Exit Sub
		End If
	End If
	If Request.Form("Form_GoodType") = "2" Then
		If NotReplay = 1 Then
			Call Processor_ErrMsg("<span class=redfont>此帖已锁定，无法评价.</span>")
			Exit Sub
		End If
		OpinionAnnounce()
		Exit Sub
	...
	If Request.Form("SureFlag")="1" and MakeGood_Level >= 2 Then
		If CheckWriteEventSpace() = 0 Then
			Call Processor_ErrMsg("<span class=redfont>您的操作过频，请稍候刷新再试！</span>")
			Exit Sub
		End If
		...
		Set Rs = LDExeCute(SQL,0)
		If Rs.Eof Then
			Call Processor_ErrMsg("错误，未选择要精华的帖子！" & VbCrLf)
			Rs.Close
			Set Rs = Nothing
			Exit Sub
		...
		If GoodFlag = 1 Then
			...
			Call Processor_Done("此帖子已经是精华帖，现取消精华！" & VbCrLf)
	...
	Else
		...
		If DEF_BBS_PrizeAnnouncePoints <> 0 Then%>
			...
			<hr class=splitline>
					...
					End If%>精华编号为<font color=ff0000 class=redfont><%=LngStr(LMT_AncID)%></font>的帖子</span>
					<div class=value2>
...
%>
		...
		Else%><b>确认要<%
			If GBL_GoodFlag = 1 Then%>取消<%
			End If%>精华编号为<font color=ff0000 class=redfont><%=LngStr(LMT_AncID)%></font>的帖子吗？</b>
			<br><%
		End If
...
Sub OpinionAnnounce
	...
	If LMT_UserID = 0 Then
		Call Processor_ErrMsg("<span class=redfont>游客帖子无法评价.</span>")
		Exit Sub
	End If
	Dim Form_Opinion
	...
	If MakeGood_Level >= 1 Then
		...
		If DEF_AllowPunish = 1 Then
			If MakeGood_Level = 1 and Form_AddPoints = 0 Then
				Call Processor_Done("成功操作，但用户" & DEF_PointsName(0) & "无任何改动．")
				Exit Sub
			End If
		Else
		...
		If MakeGood_Level = 1 and GBL_CHK_Points < Abs(Form_AddPoints + 0) Then
			Call Processor_Done("您的" & DEF_PointsName(0) & "不足, 无法完成此次评价．")
			Exit Sub
		End If
	Else
		Form_AddPoints = 0
