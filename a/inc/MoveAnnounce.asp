...
Function CheckMoveSure
	...
	If GetBinarybit(GBL_Board_BoardLimit,8) = 1 Then
		Call Processor_ErrMsg("此版面不允许转移或镜像帖子！")
		CheckMoveSure = 0
		Exit Function
	End If
...
Sub Process_MoveAnnounce(MoveID)
	...
	If Rs.Eof Then
		Call Processor_ErrMsg("未选择要操作的帖子！" & VbCrLf)
		Rs.Close
		Set Rs = Nothing
		Exit Sub
	End if
	...
	If CheckLimitNameOnly(MoveUserName) = 1 and GBL_BoardMasterFlag < 9 and (CheckSupervisorNameOnly() = 0 or GBL_UserID < 1) then
		Call Processor_ErrMsg("编号" & MoveID & "帖子无足够权限操作。")
		Exit Sub
	end if
		...
		If BoardID2 < 1 Then
			Call Processor_ErrMsg("错误，目标论坛不存在！" & VbCrLf)
			Exit Sub
		End if
		
		If BoardID2 = BoardID Then
			Call Processor_ErrMsg("目标论坛就是主题所在论坛，操作忽略！" & VbCrLf)
			Exit Sub
		End if
		...
		If isArray(Temp) = False Then
			If BoardID2 = 444 Then
				Call Processor_ErrMsg("论坛回收站未建立，请联系管理员！" & VbCrLf)
			Else
				Call Processor_ErrMsg("论坛发生错误，请联系管理员！" & VbCrLf)
			End If
			Exit Sub
		...
		If GetBinarybit(BoardLimit2,12) = 1 Then
			Call Processor_ErrMsg("目标版面<u>属于分类论坛</u>，不允许此操作。" & VbCrLf)
			Exit Sub
		End If
		...
		If Action_Str = "mirror" Then
			If cCur(LngStr(GetData(17,0))) = 39 Then
				Call Processor_ErrMsg("此帖<u>已是镜像帖子</u>，无法再次镜像。" & VbCrLf)
				Exit Sub
			End If
			...
			Call Processor_Done("<span class=greenfont>原主题成功镜像到" & BoardName2 & "！</span>" & VbCrLf)
			Exit Sub
		End If
		...
		CALL LDExeCute("insert into LeadBBS_Log(LogType,LogTime,LogInfo,UserName,IP,BoardID) Values(103," & GetTimeValue(DEF_Now) & ",'" & Left("成功转移编号" & LngStr(MoveID) & "，原作者编号" & UserID & "的帖子．原版面编号" & BoardID & "(" & Replace(Replace(htmlencode(BoardName),"\","\\"),"'","''") & ")，目标论坛编号" & BoardID2 & "(" & Replace(Replace(htmlencode(BoardName2),"\","\\"),"'","''") & "．",255) & "','" & Replace(GBL_CHK_User,"'","''") & "','" & Replace(GBL_IPAddress,"'","''") & "'," & GBL_Board_ID & ")",1)
		...
		LMT_AncID = 0
		Call Processor_Done("<span class=greenfont>原主题成功转移到" & BoardName2 & "！</span>" & VbCrLf)
End Sub

Function DisplayMoveAnnounce

	If cStr(LMT_AncID) = "0" Then
		Call Processor_ErrMsg("错误，未选择要操作的帖子！" & VbCrLf)
		Exit Function
	End if
