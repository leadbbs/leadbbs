...
<p>注：附件大小限制为 <%=int(DEF_FileMaxBytes/1024)%>K，其中图片最大 <%=int(DEF_ImageMaxBytes/1024)%>K<%
...
private Function GetSaveFileName(fname)
	...
	' README §47: images/upload/face/ is not in the upstream zip (a zip cannot carry an empty
	...
	' one level, hence the helper. §6: it is defined below, so the call needs parentheses.
...
Public Sub Upload_File
	...
	if GBL_filelistArr(NN) <> "" then
		...
		If FileName <> "" Then
			...
			If Save_Type = 0 and FileSize > DEF_ImageMaxBytes Then '图片单独限制，见 DEF_ImageMaxBytes
